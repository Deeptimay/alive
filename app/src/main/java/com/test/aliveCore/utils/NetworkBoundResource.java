...
// T: Type for the Resource data. (database cache)
// v: Type for the API response. (network request)
public abstract class NetworkBoundResource<T, V> {
    ...
    private String getCustomErrorMessage(Throwable error) {
        ...
        if (error instanceof SocketTimeoutException) {
            return MyApplication.getInstance().getString(R.string.requestTimeOutError);
        } else if (error instanceof MalformedJsonException) {
            return MyApplication.getInstance().getString(R.string.responseMalformedJson);
        } else if (error instanceof IOException) {
            return MyApplication.getInstance().getString(R.string.networkError);
        } else if (error instanceof HttpException) {
            return (((HttpException) error).response().message());
        } else {
            return MyApplication.getInstance().getString(R.string.unknownError);
        }
    }
    ...
    @MainThread
    private boolean shouldFetch(LiveData<T> dbSource) {
        List<Datum> data = (List<Datum>) dbSource.getValue();
        return data.size() == 0;
    }
